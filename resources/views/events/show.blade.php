...
<head>
    ...
    @php
        $yesClass = 'px-4 py-2 border rounded-full';
        $maybeClass = 'px-4 py-2 border rounded-full';
        $noClass = 'px-4 py-2 border rounded-full';

        if ($event->id === 0) {
            $maybeClass = 'px-4 py-2 bg-purple-600 rounded-full text-white';
        } elseif ($event->id % 2 === 0) {
            $yesClass = 'px-4 py-2 bg-purple-600 rounded-full text-white';
        } else {
            $noClass = 'px-4 py-2 bg-purple-600 rounded-full text-white';
        }
    @endphp
</head>
            ...
            <div class="p-2 pt-2 space-y-4 px-6">
                ...
                <div class="text-center mt-6">
                    ...
                    <div class="flex justify-around text-sm font-semibold">
                        <button class="{{ $yesClass }}">Yes</button>
                        <button class="{{ $maybeClass }}">Maybe</button>
                        <button class="{{ $noClass }}">No</button>
                    </div>
                </div>
                ...
                <div class="flex items-center mt-6">
                    ...
                    <img src="https://randomuser.me/api/portraits/men/78.jpg"
                        class="w-8 h-8 rounded-full border-2 border-white -ml-3">
                    <span class="text-xs ml-3 font-sans">silvia1, silvia2, silvia3 and others are coming to this
                        event</span>
                </div>
            </div>
        </div>
